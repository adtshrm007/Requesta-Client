    ...
    <div className="mt-16 bg-[#0B0F19]/60 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] overflow-hidden relative shadow-[0_0_80px_rgba(30,58,138,0.1)] group/main mx-auto max-w-7xl transition-all duration-700">
      ...
      <div className="relative z-20 px-10 py-8 border-b border-white/[0.06] flex flex-col lg:flex-row lg:items-center justify-between gap-8 bg-gradient-to-b from-white/[0.02] to-transparent">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 rounded-[1.5rem] bg-gradient-to-tr from-violet-600/30 to-indigo-600/30 border border-white/10 flex items-center justify-center shadow-2xl relative group overflow-hidden">
            <Brain size={28} className="text-white relative z-10 drop-shadow-[0_0_10px_rgba(255,255,255,0.4)]" />
          </div>
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              <h2 className="font-growmajour text-2xl md:text-3xl text-white tracking-tight">Leave Intelligence</h2>
              <div className="px-2.5 py-0.5 rounded-full bg-violet-500/10 border border-violet-500/20 flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-violet-400 animate-pulse" />
                <span className="text-[10px] font-bold text-violet-400 uppercase tracking-widest leading-none">AI Powered</span>
              </div>
            </div>
            <p className="text-white/30 font-mooxy text-[11px] uppercase tracking-[0.25em]">
              Role: <span className="text-white/60">{role || "Admin"}</span> · AI analyzes real leave data
            </p>
          </div>
        </div>
        ...
        <button onClick={fetchInsights} disabled={loading}
          ...
          <div className="relative z-10 flex items-center justify-center gap-3">
            {loading ? <Loader2 size={16} className="animate-spin text-violet-400" /> : <RefreshCw size={16} className="text-white/50 group-hover/btn:rotate-180 transition-transform duration-700" />}
            <span>Analyze Leave Patterns</span>
          </div>
        </button>
      </div>
          ...
          <div className="space-y-10 animate-in fade-in zoom-in-95 duration-1000 ease-out">
            ...
            {data.executiveSummary && (
              <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-stretch">
                {/* Health Card */}
                <div className="xl:col-span-4 bg-gradient-to-br from-white/[0.04] to-transparent border border-white/[0.08] rounded-[2rem] p-8 flex flex-col justify-center relative overflow-hidden">
                  ...
                  <div className="mb-6 flex items-center gap-3">
                    ...
                    <span className={`font-mooxy font-black text-sm tracking-widest uppercase ${
                  ...
                  <h3 className="text-white font-growmajour text-2xl leading-tight mb-3">{data.executiveSummary.headline}</h3>
                  <p className="text-white/30 font-mooxy text-[11px] uppercase tracking-wider">AI-Generated Summary</p>
                </div>

                {/* Summary + Risk + Action */}
                <div className="xl:col-span-8 bg-white/[0.02] border border-white/[0.06] rounded-[2rem] p-10 flex flex-col justify-center space-y-8">
                  <div className="relative">
                    <Sparkles size={16} className="text-violet-400/30 absolute -top-4 -left-4" />
                    <p className="text-white/80 font-mooxy text-lg leading-relaxed italic">"{data.executiveSummary.summary}"</p>
                  </div>
            ...
            {data.leaveReasonInsights?.length > 0 && (
              <div className="bg-white/[0.02] border border-white/[0.06] rounded-[2rem] p-8 relative overflow-hidden">
                ...
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h3 className="text-white font-bold text-[13px] uppercase tracking-[0.15em]">Why People Apply Leave</h3>
                    <p className="text-white/25 font-mooxy text-[10px]">AI analysis of leave reasons with actual numbers</p>
                  </div>
                </div>
                  ...
                  {data.leaveReasonInsights.map((r, i) => {
                    ...
                    return (
                      <div key={i} className="group/reason">
                        ...
                        {/* AI insight */}
                        <div className="flex gap-2 items-start ml-11">
                          <MessageSquare size={10} className="text-violet-400/40 mt-0.5 shrink-0" />
                          <p className="text-white/40 font-mooxy text-[11px] leading-relaxed italic">{r.insight}</p>
                        </div>
                      </div>
                    );
                  })}
            ...
            {data.monthlyInsights?.length > 0 && (
              <div className="bg-white/[0.02] border border-white/[0.06] rounded-[2rem] p-8 relative overflow-hidden">
                ...
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h3 className="text-white font-bold text-[13px] uppercase tracking-[0.15em]">Monthly Leave Trends</h3>
                    <p className="text-white/25 font-mooxy text-[10px]">AI interpretation of leave volume by month</p>
                  </div>
                </div>
                  ...
                  {data.monthlyInsights.map((m, i) => {
                    ...
                    return (
                      <div key={i} className="group/month">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-3">
                            <span className="text-white/80 font-mooxy text-[14px] font-bold min-w-[70px]">{formatMonth(m.month)}</span>
                            <span className="text-sky-400 font-bold text-[12px] tabular-nums">{total} leaves</span>
                          </div>
                        ...
                        <div className="h-[6px] bg-white/[0.04] rounded-full overflow-hidden flex mb-2">
                        ...
                        <div className="flex gap-2 items-start">
                          <MessageSquare size={10} className="text-violet-400/40 mt-0.5 shrink-0" />
                          <p className="text-white/40 font-mooxy text-[11px] leading-relaxed italic">{m.insight}</p>
                        </div>
                      </div>
                    );
                  })}
                  ...
                  <div className="flex flex-wrap gap-4 pt-4 border-t border-white/[0.05]">
                    {[{ label: "Approved", color: "bg-emerald-500" }, { label: "Rejected", color: "bg-rose-500" }, { label: "Pending", color: "bg-amber-500" }].map(l => (
                      <div key={l.label} className="flex items-center gap-1.5">
                        ...
                        <span className="text-white/25 font-mooxy text-[10px]">{l.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
